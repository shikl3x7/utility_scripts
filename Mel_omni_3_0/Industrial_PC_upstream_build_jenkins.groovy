pipeline {
    ...
    parameters {
        ...
        string(name: 'MACHINE', defaultValue: 'industrial-pc' , description: 'Name of the Machine')
        string(name: 'BUILD_IMAGES', defaultValue: 'service-stick-image development-ade' , description: 'Images to be built')
        string(name: 'INPFTP_URI', defaultValue: 'inpftp@inpftp.ina.mentorg.com:/home/inpftp/pub/OMNI_3.0_GA_UPSTREAM/industrial_pc' , description: 'Location where build artifacts are copied to')
            ...
            steps {
                script {
                    sh '''#!/bin/bash
                        ...
                        echo 'IMAGE_FEATURES_append = " mtda "' >> \${BUILD_PATH}/\${IMAGE_ID}/conf/local.conf
                        ...
                        sshpass -p "inpftp" ssh inpftp@inpftp.ina.mentorg.com 'rm -rf $(ls -d -1t /home/inpftp/pub/OMNI_3.0_GA_UPSTREAM/industrial_pc/* | tail -n +2)'
                        echo -e "\n#### Copying service-stick-image and ADE to inpftp server ####\n"
                        # Copy service-stick-image to inpftp server
                        sshpass -p "inpftp" rsync tmp/deploy/images/industrial-pc/service-stick-image-mel-omni-industrial-pc.wic.img \${INPFTP_URI}/\${IMAGE_ID}/
                        sshpass -p "inpftp" rsync tmp/deploy/images/industrial-pc/development-image-mel-omni-industrial-pc.wic.img \${INPFTP_URI}/\${IMAGE_ID}/
                        #sshpass -p "inpftp" rsync tmp/deploy/ade/*.zip \${INPFTP_URI}/\${IMAGE_ID}/
                        sshpass -p "inpftp" rsync tmp/deploy/images/industrial-pc/*.manifest \${INPFTP_URI}/\${IMAGE_ID}/
                    '''
                }
            }
        }
